...
stamps (``version_findings``: V001 for a ``format`` other than ``khg-queue/1.0.x`` or ``1.1.x``, or a
``record_format`` other than ``khg-record/1.0.x`` or ``1.1.x``, DESIGN §11.2; nothing else is checked then, as a V
failure stops a validator run). Then it runs, line by line, the queue schema and ``Fold.feed``, which reports:
...
def version_findings(header: Mapping[str, Any]) -> list[Finding]:
    ...
    stamp): a ``format`` other than ``khg-queue/1.0.x`` or ``1.1.x`` (ruling 23), and a ``record_format``, when there
    is one, other than ``khg-record/1.0.x`` or ``1.1.x`` (ruling 22; a missing one is the schema's Q008)."""
    out = []
    for name, want, required, minor in (("format", "khg-queue", True, 1), ("record_format", "khg-record", False, 1)):
