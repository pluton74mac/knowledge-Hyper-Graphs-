...
def test_the_hif_store_loads_a_container_that_is_not_complete(schema, fixture_doc):
    ...
    try:
        ...
        quoted = Where(status=frozenset({"asserted", "quoted"}))  # f:born-louis14-paris is quoted
        assert [r["id"] for r in s.incident("ex:Paris", where=quoted)] == [
            r["id"] for r in reference.incident("ex:Paris", where=quoted)] == ["f:born-louis14-paris"]
    finally:
        s.close()
